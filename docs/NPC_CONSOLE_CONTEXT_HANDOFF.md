...
The top-left console button wears an avatar for the provider behind the current
model, with the provider and the exact model identifier in its tooltip. The
mapping lives in `packages/autorio/src/task_board_provider.ts`, driven by the
`provider_model` the runtime already reports in its debug snapshot, and falls
back to the neutral AIRI avatar for an unrecognized identifier.

The avatars are original drawings generated by
`packages/autorio/scripts/generate_provider_icons.py` and declared as sprite
prototypes in `data.lua`. The mod ships no vendor artwork or logo: a provider is
identified by its accent color and a plain charm. Regenerate with the script
rather than hand-editing the PNGs, and keep `data.lua` and the resolver's ids in
step - a control-stage id with no matching prototype leaves the button blank.
