...
const CHANNEL = Object.freeze({
  ...
  sourceRef: 'feat/swarm-pterodactyl-playable',
  release: 'swarm-e2e',
})
...
function assertNpcV8Source(source) {
  ...
  if (!text.includes('AIRI_CHAT_PLAYERS')) failures.push('missing AIRI_CHAT_PLAYERS')
...
export function channelInstaller(source) {
  assertNpcV8Source(source)
  const sourceHash = sha256(source)
  return `#!/usr/bin/env bash
...
DEFAULT_SOURCE_REF="${CHANNEL.sourceRef}"
...
function egg(installScript) {
  return {
    ...
    variables: [
      variable('AIRI Source Ref', 'Git branch, tag, or exact 40-character commit SHA followed when this swarm egg is reinstalled. Restart does not resolve or update this ref.', 'AIRI_SOURCE_REF', CHANNEL.sourceRef, 'required|string|max:200'),
      variable('AIRI Actor Mode', 'Controlled actor mode. The chat supervisor remains one standalone NPC while the installed Autorio runtime is swarm-capable.', 'AIRI_ACTOR_MODE', 'npc', 'required|string|in:npc'),
      ...
      variable('Jev Decision API Key', 'Optional credential for the global Project Jev decision provider. Leave blank to keep Project Jev unavailable without blocking swarm runtime.', 'DECISION_PROVIDER_API_KEY', '', 'nullable|string|max:512', { viewable: false }),
      variable('Jev Decision Provider URL', 'Decision-provider endpoint used by Project Jev. Remote endpoints must use HTTPS.', 'DECISION_PROVIDER_API_URL', 'https://api.typesafe.ai/v1/systemone', 'required|string|url|max:255'),
      variable('Jev Decision Model', 'Model identifier used by the global Project Jev control plane.', 'DECISION_PROVIDER_MODEL', 'jev-latest', 'required|string|max:200'),
      variable('Jev Decision Timeout (ms)', 'Maximum time for one Project Jev decision request.', 'DECISION_PROVIDER_TIMEOUT_MS', '5000', 'required|numeric|between:100,30000'),
      variable('Max Jev Decisions Per Hour', 'Persisted hourly request cap for the Project Jev decision provider, separate from the main AIRI provider budget.', 'MAX_DECISION_PROVIDER_REQUESTS_PER_HOUR', '180', 'required|numeric|between:1,1200'),
...
export function buildArtifacts(source) {
  ...
  const eggJson = `${JSON.stringify(egg(channelInstaller(source)), null, 4)}\n`
...
export function verifyGeneratedArtifacts(source, installText, eggText) {
  ...
  const sourceRef = parsedEgg.variables?.find(entry => entry.env_variable === 'AIRI_SOURCE_REF')
  if (sourceRef?.default_value !== CHANNEL.sourceRef) throw new Error('Generated swarm egg has the wrong source ref')
  if (!parsedEgg.scripts?.installation?.script?.includes(`CHANNEL="${CHANNEL.release}"`)) throw new Error('Generated swarm egg has the wrong channel loader')
  return true
}
