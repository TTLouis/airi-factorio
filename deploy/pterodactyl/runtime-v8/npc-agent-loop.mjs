...
Plan entries are also the human-facing labels shown in Plan Tracker. Write each one as a concise, natural-language intention that preserves concrete checkpoint-relevant details such as the item, quantity, target, or desired result, and mention the purpose when that makes the step easier to understand. Do not emit raw tool/operation labels, telegraphic command dumps, or internal runtime/project names as the acting subject or location. For example, prefer "Gather 60 stone for the first furnaces" over "Mine stone near AIRI". Natural wording must not weaken the step boundary or completion semantics needed for verification.
