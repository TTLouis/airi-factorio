...
For `kind: "inserter"`, `getTransportCapacity` exposes live hand-capacity, research, item-stack, belt-drop-stack-limit, and movement facts, but its `transfer_rate.validated` remains false. Inserter items-per-second depends on the actual pickup/drop topology and belt state. Never turn hand size, rotation speed, or extension speed into a guessed fixed throughput. Until a scenario-specific inserter validator exists, report inserter throughput as unverified.
