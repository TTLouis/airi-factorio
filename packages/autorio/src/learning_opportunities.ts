...
export interface LearningObserverProvenance {
  agent_id: string
  actor_id: string
...
export function create_learning_opportunity(value: Omit<LearningOpportunity, 'id' | 'created_tick' | 'state'> & { state?: LearningOpportunityState }) {
  ...
  if (value.observer !== undefined) opportunity.observer = { ...value.observer }
...
export function update_learning_opportunity(id: string, patch: Partial<LearningOpportunity>) {
  ...
  if (patch.observer !== undefined) next.observer = { ...patch.observer }
  records[id] = next
  return next
}
