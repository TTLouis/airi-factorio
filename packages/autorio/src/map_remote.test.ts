...
describe('map-first remote control', () => {
  ...
  it('refuses map queries when none of the requested chunks are charted', () => {
    ...
    expect(result).toMatchObject({ ok: false, code: 'area_uncharted', entities: [], charted_chunks: 0 })
  ...
  it('does not read live entities from charted chunks that are currently fogged', () => {
    ...
    expect(result).toMatchObject({ ok: false, code: 'area_not_visible', charted_chunks: 1, visible_chunks: 0, entities: [] })
    expect(surface.find_entities_filtered).not.toHaveBeenCalled()
  })
  ...
  it('refuses exact entity inspection for charted but currently invisible entities', () => {
    ...
    expect(inspect_charted_entity(actor, 42)).toEqual({ ok: false, code: 'area_not_visible', unit_number: 42 })
  })
  ...
  it('sets an enabled compatible machine recipe remotely without physical-distance checks', () => {
    ...
    ;(actor.force.recipes as any)['electronic-circuit'] = { name: 'electronic-circuit', enabled: true, categories: ['crafting'] }
    const entity = make_entity(surface)
    ...
    expect(result).toMatchObject({ accepted: true, completed: true, code: 'completed', execution_mode: 'remote', unit_number: 42, recipe_name: 'electronic-circuit', removed_item_count: 0 })
  })
  ...
  it('rejects a recipe change before Lua can displace recipe-dependent machine contents', () => {
    ...
    const actor = make_actor(surface, new Set(['0,0']))
    ;(actor.force.recipes as any)['electronic-circuit'] = { name: 'electronic-circuit', enabled: true, categories: ['crafting'] }
    const entity = make_entity(surface, { get_inventory: vi.fn(() => ({ is_empty: vi.fn(() => false) }) as any) })
    ;(globalThis as any).game.get_entity_by_unit_number.mockReturnValue(entity)

    const result = set_charted_machine_recipe(actor, 42, 'electronic-circuit')

    expect(result).toMatchObject({ accepted: false, completed: false, code: 'recipe_change_unsafe', execution_mode: 'remote' })
  ...
  it('keeps remote mutations inside the owning force', () => {
    ...
    expect(result).toMatchObject({ accepted: false, completed: false, code: 'wrong_force', execution_mode: 'remote' })
    expect(entity.set_recipe).not.toHaveBeenCalled()
  })
})
