...
function owner(task: BasicTask) {
  return { actor_id: task.owner_actor_id, actor_kind: task.owner_actor_kind, force_index: task.owner_force_index }
}
...
function last_result(persistenceKey?: string) {
  return persistenceKey === undefined ? storage.airi_last_basic_operation_result : storage.airi_basic_operation_contexts?.[persistenceKey]?.last_result
}
...
function bind(task: BasicTask, actor: ControlledActor, persistenceKey?: string) {
  ...
  if (identity.actor_id === undefined) return false
...
export function basic_identity_matches(actor: ControlledActor, task: BasicTask) {
  if (task.owner_actor_id === undefined || task.owner_actor_kind === undefined || task.owner_force_index === undefined) return false
  const identity = actor.status_snapshot()
  return identity.actor_id === task.owner_actor_id && identity.kind === task.owner_actor_kind && actor.force.index === task.owner_force_index
}
...
function result_for(actor: ControlledActor | undefined, task: BasicTask | undefined, accepted: boolean, completed: boolean, code: BasicOperationCode, details: Partial<BasicOperationResult> = {}, persistenceKey?: string) {
  ...
  const result: BasicOperationResult = {
    ...
    requested_position: task?.type === TaskStates.PLACING && task.position ? { x: task.position.x, y: task.position.y } : undefined,
...
export function new_basic_operation_controller(get_actor: () => ControlledActor | undefined, manager: ReturnType<typeof new_task_manager>, options: BasicOperationControllerOptions = {}) {
  ...
  function submit_mining(entity_name: string, count: number = 1) {
    ...
    return queue({ type: TaskStates.MINING, entity_name, count, requested_count: count }, actor)
  }
  function submit_placement(entity_name: string, x?: number, y?: number, direction?: number) {
    ...
    if (has_x !== has_y || (has_x && (!valid_coordinate(x!) || !valid_coordinate(y!)))) {
      result_for(get_actor(), undefined, false, false, 'invalid_position', {}, persistenceKey)
      return false
    }
    if (direction !== undefined && !valid_integer(direction, 0, 15)) {
      result_for(get_actor(), undefined, false, false, 'invalid_direction', {}, persistenceKey)
      return false
    }
    const actor = actor_for_submission()
    if (!actor) return false
    return queue({ type: TaskStates.PLACING, entity_name, position: has_x ? { x: x!, y: y! } : undefined, direction }, actor)
  }
  function submit_move(item_name: string, entity_name: string, max_count: number, to_entity: boolean): [boolean, string] {
    ...
    if (!queue({ type: TaskStates.MOVING_ITEMS, item_name, entity_name, max_count, to_entity }, actor)) return [false, 'No controlled actor']
  ...
  function submit_move_exact(item_name: string, target_unit_number: number, max_count: number, to_entity: boolean): [boolean, string] {
    if (!valid_integer(target_unit_number, 1, 9007199254740991)) {
      result_for(get_actor(), undefined, false, false, 'invalid_unit_number', {}, persistenceKey)
    ...
    const actor = actor_for_submission()
    if (!actor) return [false, 'No controlled actor']
    if (!queue({ type: TaskStates.MOVING_ITEMS, item_name, target_unit_number, max_count, to_entity }, actor)) return [false, 'No controlled actor']
    return [true, 'Task started']
  }
  function submit_set_recipe_exact(target_unit_number: number, recipe_name: string): [boolean, string] {
    ...
    if (!valid_name(recipe_name)) {
      result_for(get_actor(), undefined, false, false, 'invalid_recipe', {}, persistenceKey)
      return [false, 'recipe_name must be a valid Factorio recipe name']
    }
    const actor = actor_for_submission()
    if (!actor) return [false, 'No controlled actor']
    if (!queue({ type: TaskStates.SETTING_RECIPE, target_unit_number, recipe_name }, actor)) return [false, 'No controlled actor']
    return [true, 'Task started']
  }
  function submit_player_move(item_name: string, player_name: string, max_count: number, to_player: boolean): [boolean, string] {
    ...
    if (typeof player_name !== 'string' || player_name.length === 0) {
      result_for(get_actor(), undefined, false, false, 'player_unavailable', {}, persistenceKey)
      return [false, 'player_name is required']
    }
    const actor = actor_for_submission()
    if (!actor) return [false, 'No controlled actor']
    if (!queue({ type: TaskStates.MOVING_ITEMS, item_name, player_name, max_count, to_player }, actor)) return [false, 'No controlled actor']
    return [true, 'Task started']
  }
  function submit_wait(ticks: number): [boolean, string] {
    ...
    const actor = actor_for_submission()
    if (!actor) return [false, 'No controlled actor']
    if (!queue({ type: TaskStates.WAITING, remaining_ticks: ticks, requested_ticks: ticks }, actor)) return [false, 'No controlled actor']
    return [true, 'Task started']
  }
  ...
  function register_cancel(state: TaskStates.MINING | TaskStates.PLACING | TaskStates.MOVING_ITEMS | TaskStates.SETTING_RECIPE | TaskStates.WAITING, get_task: () => BasicTask | undefined) {
    manager.register_cancel_handler(state, () => {
      ...
      const task = get_task()
      if (!task) return
      result_for(get_actor(), task, false, false, 'cancelled', {}, persistenceKey)
    })
  }
  ...
  function status() {
    return {
      ...
      next_operation_id: persistenceKey === undefined ? (storage.airi_next_basic_operation_id ?? 0) : (storage.airi_basic_operation_contexts?.[persistenceKey]?.next_operation_id ?? 0),
    }
  }
