...
describe('deterministic work selection', () => {
  ...
  it('scores the actors own active claim for continuity but never another actors claim', () => {
    ...
    const claimed = claim_work(swarm, { workId: current.id, expectedRevision: current.revision, actor: { agentId: agent.id, actorId: 'actor-1', bodyRevision: 1, available: true, capabilities: ['move'] }, tick: 2, leaseTicks: 100 })
    ...
    const scores = rank_work_candidates(swarm, { agentId: agent.id, actorId: 'actor-1', capabilities: ['move'], location: { surfaceIndex: 1, position: { x: 0, y: 0 } }, inventory: {}, currentWorkId: current.id })
    expect(scores).toHaveLength(1)
